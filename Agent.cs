    ...
    public abstract class Agent : MessageComponent
    {
        ...
        public virtual void Perform()
        {
            ...
            else
            {
                ...
                **/
                if (Mouse.current.rightButton.wasPressedThisFrame && Physics.Raycast(Manager.SelectedCamera.ScreenPointToRay(new(Mouse.current.position.x.ReadValue(), Mouse.current.position.y.ReadValue(), 0)), out RaycastHit hit, Mathf.Infinity, Manager.GroundLayers | Manager.ObstacleLayers))
                {
                    GameObject agentObj = GameObject.FindGameObjectWithTag("Agent"); // Find the agent game object with the "Agent" tag

                    if (agentObj != null)
                    {
                        Agent agent = agentObj.GetComponent<Agent>(); // Get the agent component of the agent game object

                        GameObject newAgentObj = Instantiate(agentObj, hit.point, Quaternion.identity); // Create a new game object with the same components and settings as the original game object
                        Agent newAgent = newAgentObj.GetComponent<Agent>(); // Get the agent component of the new game object
                        newAgent.Navigate(GetRandomPosition(newAgent.transform.position)); // Make the new agent navigate to a random position

                        GameObject[] allAgents = GameObject.FindGameObjectsWithTag("Agent"); // Find all agents in the scene with the "Agent" tag

                        foreach (GameObject otherAgentObj in allAgents)
                        {
                            if (otherAgentObj != agentObj && otherAgentObj != newAgentObj)
                            {
                                Agent otherAgent = otherAgentObj.GetComponent<Agent>(); // Get the agent component of the other agent game object
                                otherAgent.Navigate(GetRandomPosition(otherAgent.transform.position)); // Make the other agent navigate to a random position
                            }
                        }
                    }
                }
        ...
        private Vector3 GetRandomPosition(Vector3 position)
        {
            ...
            UnityEngine.AI.NavMesh.SamplePosition(randomDirection, out UnityEngine.AI.NavMeshHit hit, 10f, UnityEngine.AI.NavMesh.AllAreas);
            return hit.position;
        }
